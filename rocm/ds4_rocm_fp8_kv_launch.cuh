extern "C" int ds4_gpu_dsv4_fp8_kv_quantize_tensor(ds4_gpu_tensor *x, uint32_t n_tok, uint32_t head_dim, uint32_t n_rot) {
    ...
    fp8_kv_quantize_kernel<<<dim3(n_tok, groups), 64>>>(
            (float *)x->ptr, NULL, n_tok, head_dim, n_rot);
...
extern "C" int ds4_gpu_dsv4_fp8_kv_quantize_mirror_f16_tensor(
        ...
        uint32_t n_rot) {
    if (n_rot > head_dim ||
        !cuda_tensor_has_elems2(x, n_tok, head_dim, sizeof(float)) ||
        !cuda_tensor_has_elems2(mirror_f16, n_tok, head_dim, sizeof(half))) {
        return 0;
    }
    if (n_tok == 0u || head_dim == 0u) return 1;
    ...
    fp8_kv_quantize_kernel<<<dim3(n_tok, groups), 64>>>(
            ...
            head_dim,
            n_rot);
    return cuda_ok(cudaGetLastError(), "fp8_kv_quantize mirror f16 launch");
}
